...
#endif
                // For IMA (bcovViewController), add ad tags to cue points
                let video = controlType == .bcovViewController 
                    ? videoItem.video.updateVideo(useAdTagsInCuePoints: true)
                    : videoItem.video
                playbackController.setVideos([video])
                didSetVideo.toggle()
            }
            // Resume ad playback if using IMA
            if controlType == .bcovViewController {
