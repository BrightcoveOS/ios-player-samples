...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        self.castMediaController = [GCKUIMediaController new];
        self.castMediaController.delegate = self;

        self.posterImageSize = CGSizeMake(480, 720);
    }

    return self;
}

- (BCOVSource *)findPreferredSourceFromSources:(NSArray<BCOVSource *> *)sources
                                     withHTTPS:(BOOL)withHTTPS
{
    ...
    for (BCOVSource *source in filteredSources)
    {
        ...
        if ([urlString containsString:@"hls/v3"] &&
            [deliveryMethod isEqualToString:@"application/x-mpegURL"])
        {
            hlsSource = source;
        ...
        if ([urlString containsString:@"v1/dash"] &&
            [deliveryMethod isEqualToString:@"application/dash+xml"])
        {
            dashSource = source;
        }
...
- (void)createMediaInfoFromVideo:(BCOVVideo *)video
{
    ...
    source = [self findPreferredSourceFromSources:video.sources
                                        withHTTPS:YES];
    ...
    if (poster)
    {
        ...
        if (imageURL)
        {
            [metadata addImage:[[GCKImage alloc] initWithURL:imageURL
                                                       width:self.posterImageSize.width
                                                      height:self.posterImageSize.height]];
        }
    }
    ...
    for (NSDictionary *track in textTracks)
    {
        ...
        if ([kind isEqualToString:@"captions"] || [kind isEqualToString:@"subtitles"])
        {
            ...
            [[GCKMediaTrack alloc] initWithIdentifier:trackIdentifier
    ...
    GCKMediaInformationBuilder *builder = [GCKMediaInformationBuilder new];
...
- (void)setupRemoteMediaClientWithMediaInfo
{
    ...
    if (castSession)
    {
        [castSession.remoteMediaClient loadMedia:self.castMediaInfo
                                     withOptions:options];
    }
}
